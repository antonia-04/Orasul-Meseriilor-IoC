...
const initialTransforms = {
  sun: "translate(180, 130)",
  drops: {
    drop1: "translate(150, 300)",
    drop2: "translate(250, 415)",
    drop3: "translate(750, 300)",
    drop4: "translate(850, 200)",
    drop5: "translate(550, 400)",
    drop6: "translate(100, 150)"
  },
  rocks: {
    rock1: "translate(550, 220)",
    rock2: "translate(400, 320)",
    rock3: "translate(200, 300)",
    rock4: "translate(300, 260)",
    rock5: "translate(100, 300)"
  }
};
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // speech.js
          say(
            "Bună! Eu sunt Strop. Hai să explorăm împreună circuitul apei în natură! Apasă butonul pentru a începe jocul."
          );
        }
      });
    },
...
function goToStage(n) {
  ...
  // Reset stage state (important for hints + counters)
  resetStage(n);
  ...
  if (n === 1) {
    ...
    say("Precipitațiile. Ține apăsat pe nor pentru a începe ploaia!");
  } else if (n === 4) {
...
function getStageName(n) {
  switch (n) {
    case 1:
      return "1. Evaporarea";
    case 2:
      return "2. Condensarea";
    case 3:
      return "3. Precipitațiile";
    case 4:
      return "4. Colectarea";
    default:
      return "";
...
function resetStage(n) {
  if (n === 1) {
    // Reset steam + message + sun position + show hints
  ...
  if (n === 2) {
    // Reset drops + counter + cloud target + instruction + hints
    dropsCount = 0;
    ...
    if (target) {
      ...
      target.setAttribute("stroke", "#A9D0F5");
    }
    ...
    Object.keys(initialTransforms.drops).forEach((id) => {
      ...
      drop.removeAttribute("data-moved");
      drop.setAttribute("transform", initialTransforms.drops[id]);
    });
  ...
  if (n === 3) {
    // Reset rain + gauge + sky + hints
    rainTime = 0;
    stopRain();

    const water = document.getElementById("water-level");
    if (water) {
      water.setAttribute("height", 200);
  ...
  if (n === 4) {
    // Reset rocks + flow + counter + hints
    rocksCount = 0;
    ...
    if (water) {
      water.setAttribute("stroke-dashoffset", "1600");
    }

    Object.keys(initialTransforms.rocks).forEach((id) => {
      const rock = document.getElementById(id);
      if (!rock) return;
      rock.style.display = "";
      rock.style.opacity = "";
      rock.style.transform = "";
      delete rock.dataset.moved;
      rock.setAttribute("transform", initialTransforms.rocks[id]);
    });
...
function endDrag(evt) {
  ...
  if (dragType === "sun" && currentStage === 1) {
    if (y > 250) {
      ...
      say("Bravo! Apasă butonul pentru a continua.");
  ...
  if (dragType === "rock" && currentStage === 4) {
    if (!selectedElement.dataset.moved) {
      ...
      // keep your existing animation idea:
      selectedElement.style.transform = "scale(0.3)";
...
function startRain() {
  ...
  rainInterval = setInterval(() => {
    ...
    if (rainTime >= rainTarget) {
      ...
      say("Felicitări! Apasă butonul pentru a continua.");
      showNextButton();
    }
  }, 50);
}
